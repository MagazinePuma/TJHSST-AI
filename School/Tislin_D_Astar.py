import sys; args = sys.argv[1:]
import time

puzzles = open(args[0]).read().splitlines()
goalState = puzzles[0]

def a_star(start, goal):
    if start == goal: return [{"current": start, "parent": start, "cost": 0, "level": 0, "compact": "G"}]
    if inversion_count(start, goal): return [{"current": start, "parent": start, "cost": 0, "level": 0, "compact": "X"}] 
    current = None
    min = 0
    frontier = [{"current": start, "parent": start, "cost": 0, "level": 0, "compact": ""}]
    explored = []
    count = 0
    
    while len(frontier):
        min = 999
        minIndex = -1
        for x in range(len(frontier)):
            if frontier[x]["level"] == count and frontier[x]["cost"] <= min:
                minIndex = x
                min = frontier[x]["cost"]
        '''
            if len(frontier) == 1:
                min = frontier[x][1]
                break
            min = minimum(frontier[x][1], frontier[x + 1][1])

        for y in range(len(frontier)):
            if frontier[y][1] == min:
                current = frontier.pop(y)
                break
        '''
        if minIndex > -1:
            current = frontier.pop(x)  

        count += 1

        for child in generate_children(current["current"]):
            if child["current"] == goal:
                explored.append(current)
                child["level"] = count
                child["cost"] = count + manhattan(goal, child["current"])
                explored.append(child)
                return explored
            '''
            childExistsInExplored = filter(lambda x: x["current"] == child["current"] and x["parent"] == child["parent"], explored)
            if len(list(childExistsInExplored)):
                continue # skip child if it is in explored dictionary
            '''
            child["level"] = count
            child["cost"] = count + manhattan(goal, child["current"])
            newFrontier = filter(lambda x: x["level"] == child["level"] and x["cost"] < child["cost"], frontier)
            newExplored = filter(lambda x: x["level"] == child["level"] and x["cost"] < child["cost"], explored)

            '''
            if len(list(newFrontier)):
                continue # skip child if cost is not better on the same level in frontier

            #if not len(list(newFrontier)) and not len(list(newExplored)):
            #    frontier.append(child)
            
            frontier.append(child)
            

        currentExistsInExplored = filter(lambda x: x["current"] == current["current"] and x["parent"] == current["parent"], explored)
        if not len(list(currentExistsInExplored)):
            explored.append(current)
        else:
            [{"current": start, "parent": start, "cost": 0, "level": 0, "compact": "X"}] 
        '''

    return [{"current": start, "parent": start, "cost": 0, "level": 0, "compact": "X"}]

def generate_children(state):
    # write code here
    size = 4
    display = []

    if state.index("_") % size != 0:
        display.append({"current": swap(state, state.index("_") - 1, state.index("_")), "parent": state, "cost": 0, "level": 0, "compact": "L"})

    if (state.index("_") + 1) % size != 0:
        display.append({"current": swap(state, state.index("_"), state.index("_") + 1), "parent": state, "cost": 0, "level": 0, "compact": "R"})

    if state.index("_") >= size:
        display.append({"current": swap(state, state.index("_") - size, state.index("_")), "parent": state, "cost": 0, "level": 0, "compact": "U"})

    if not "_" in state[-size:]:
        display.append({"current": swap(state, state.index("_"), state.index("_") + size), "parent": state, "cost": 0, "level": 0, "compact": "D"})

    return display

def swap(state, i, j):
    # write code here
    return state[:i] + state[j] + state[i + 1:j] + state[i] + state[j + 1:]

def inversion_count(initial, final):
    # write code here
    count = 0
    for x in range(len(initial) - 1):
        for y in range(x + 1, len(initial)):
            if initial[x] > initial[y] and initial[x] != "_":
                count += 1
    
    '''
    countTwo = 0
    for x in range(len(final) - 1):
        for y in range(x + 1, len(final)):
            if final[x] > final[y] and final[x] != "_":
                countTwo += 1
    '''

    return (count % 2 == 0 and abs(find_level(initial.index("_")) - find_level(final.index("_"))) % 2 == 0) or (count % 2 != 0 and abs(find_level(initial.index("_")) - find_level(final.index("_"))) % 2 != 0)
    # return count

def manhattan(start, goal):
    distance = 0
    
    for x in start:
        if x != "_":
            distance += (abs(find_level(goal.index(x)) - find_level(start.index(x))) + abs(find_position(goal.index(x)) - find_position(start.index(x))))

    return distance

def find_level(idx):
    return idx // 4

def find_position(idx):
    return idx % 4

'''
def path_length(explored):
    if explored[initialState] == -1:
        return "X"

    if explored[(initialState, None)] == "G":
        return "G"

    count = 0
    initial = initialState
    goal = list(explored.keys())[len(explored) - 1]

    while True:
        initial = explored[goal]
        if initial == goal:
            return count
        goal = initial
        count += 1

    return count
'''

def display(explored):
    compactPath = ""

    for x in range(len(explored)):
        compactPath += explored[x]["compact"]

    return compactPath

def path(explored, initialState, goalState):
    '''
    if explored[(initialState, None)] == "X":
        return ["X"]

    if explored[(initialState, None)] == "G":
        return ["G"]
    '''

    result = {}
    result[goalState] = goalState
    val = initialState
    key = list(explored.keys())[len(explored) - 1]

    while True:
        val = explored[key]
        result[key] = val[1]
        if key == val:
            compactPath = list(result.values())
            compactPath.reverse()
            return compactPath

        key = val
    '''
    compactPath = list(result.values())
    compactPath.reverse()
    return compactPath
    '''

def minimum(a, b):
    if a <= b:
        return a
    else:
        return b
'''
def isPuzzleSolvable(initialState):
    puzzle_is_solvable = False
    
    number_of_inversions = inversion_count(initialState, goalState)
    is_number_of_inversions_even = number_of_inversions % 2 == 0
    blank_level_in_init = initialState.index("_") // 4
    blank_level_in_goal = goalState.index("_") // 4
    is_blank_on_even_row = abs(blank_level_in_goal - blank_level_in_init) % 2
    
    if (is_blank_on_even_row and is_number_of_inversions_even) or (not is_blank_on_even_row and not is_number_of_inversions_even):
        puzzle_is_solvable = True
    
    return puzzle_is_solvable
'''
    

def main():
    for initialStateIndex in range(0, len(puzzles)):
        initialState = puzzles[initialStateIndex]
        start_time = time.time()
        result = a_star(initialState, goalState) 
        '''
        if not isPuzzleSolvable(initialState):
            result = [{"current": initialState, "parent": initialState, "cost": 0, "level": 0, "compact": "X"}]
        else:
            result = a_star(initialState, goalState)
        '''
        run_time = round(time.time() - start_time, 2)
        if run_time == 0:
            run_time = 0.01
        # print(path(result, initialState, goalState))
        if display(result) == "G" or display(result) == "X":
            print(str(initialStateIndex) + ": " + initialState, display(result))
        else:
            print(str(initialStateIndex) + ": " + initialState + " len " + str(len(display(result)) + 1) + " in " + str(run_time) + "s: " + display(result))

if __name__ == "__main__":
    main()

# Dennis Tislin, Period 5, 2026
