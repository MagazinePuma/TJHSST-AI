...
def utility(turn, tc, state):
   ...
   for condition in tc:
      test = [state[x] for x in condition]
      test2 = set(test)
      
      if test[0] != "." and len(test2) == 1:
         if test[0] == turn:
            return 1
...
def max_value(state, turn, tc):
   ...
   for a, s in successors(state, turn):
      if turn == "X":
         v = max(v, min_value(s, "O", tc))
      else:
         v = max(v, min_value(s, "X", tc))
...
def min_value(state, turn, tc):
   ...
   for a, s in successors(state, turn):
      if turn == "X":
         v = min(v, max_value(s, "O", tc))
      else:
         v = min(v, max_value(s, "X", tc))
