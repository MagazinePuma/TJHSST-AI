...
def a_star(start, goal):
    if start == goal: return [(start, start, 0, 0, "G")]
    current = None
    min = 0
    #frontier = [{"current": start, "parent": start, "cost": 0, "level": 0, "compact": ""}]
    frontier = []
    HeapPriorityQueue().push(frontier, ((0, 0, start, [start], "")))
    explored = {frontier: manhattan(frontier, goal)}
    #count = 0

    print_log = False
    
    while frontier:
        cost, level, current, path, compact = HeapPriorityQueue().pop(frontier)
        ...
        for child, child_compact in generate_children(current):
            ...
            HeapPriorityQueue().push(frontier, (child_cost, child_level, child, path+[child], new_child_compact))
...
def inversion_count(initial, goal):
    ...
    newInitial = initial
    newInitial.replace("_", "")
    newGoal = goal
    newGoal.replace("_", "")
    inversions = 0
    
    for x in range(len(newInitial)):
        for y in range(x + 1, len(newGoal)):
            g = newGoal.index(newInitial[x])
            #if initial[x] > goal[y]:
            if newGoal.index(newInitial[y]) < g:
                inversions += 1
...
def main():
    for initialStateIndex in range(0, len(puzzles)):
        ...
        if inversion_count(initialState, goalState):
            compact_path = 'X'
