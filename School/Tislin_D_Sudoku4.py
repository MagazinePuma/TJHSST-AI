...
def shortest_domain_index(assignment, variables):
   ...
   for idx, val in enumerate(assignment):
      if val == '.':
         ...
         if dom_length > -1 and dom_length < min_dom_length:
...
def recursive_backtracking(assignment, q_table, variables, constraints):
    ...
    for value in current_ordered_domain:
       ...
       if assignment_is_valid(next_var_index, assignment, constraints):
         ...
         #variables =  {key: value[:] for key, value in variables_copy.items()}
...
def sudoku_csp(puzzle_length):
  ...
  grid_size = n
  grid_column_height = n // 3
  grid_row_length = grid_size // grid_column_height

  first_grid = []
  for i in range(grid_column_height):
  ...
  for i in range(grid_size):
    ...
    for j in first_grid:
       ...
       base = i // grid_column_height * grid_size * grid_column_height
       extra = i % grid_column_height * grid_row_length
       arr.append(j + base + extra)
    grids.append(arr)
   
  return rows + cols + grids


def find_grid(csp_table, x, len_puzzle_sqrt):
  grids = csp_table[-len_puzzle_sqrt:]
...
def sudoku_neighbors(csp_table, len_puzzle): # {0:[0, 1, 2, 3, 4, ...., 8, 9, 18, 27, 10, 11, 19, 20], 1:
  ...
  #      {x: set(csp_table[x // 9]               + csp_table[(x % 9)               + 9]               + find_grid(csp_table, x)) for x in range(81)}    # csp_table[(x % 3) + 18]
  nbrs = {x: set(csp_table[x // len_puzzle_sqrt] + csp_table[(x % len_puzzle_sqrt) + len_puzzle_sqrt] + find_grid(csp_table, x, len_puzzle_sqrt)) for x in range(len_puzzle)}    # csp_table[(x % 3) + 18]
  for val in nbrs:
     nbrs[val] = list(set(nbrs[val]))
  return nbrs


def init_vars(puzzle):
   ...
   for char in puzzle:
      if char != '.' and char not in vars:
         only_digits = False
         break

   if not only_digits:
       uppercase_letters_list = [chr(i) for i in range(ord('A'), ord('Z') + 1)]
       vars.extend(uppercase_letters_list)
...
def initialize_ds(puzzle, neighbors):
    ...
    if "A" in vars:
      ...
      q_table = {str(i): 0 for i in range(1, range_size)}
...
def checksum(solution):
   ...
   return sum([ord(c) for c in solution]) - 49*len(solution)
