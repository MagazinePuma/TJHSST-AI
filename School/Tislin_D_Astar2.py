...
def a_star(start, goal):
    ...
    while len(frontier):
        ...
        for child in generate_children(current["current"]):
            ...
            existsInExplored = filter(lambda x: x["current"] == child["current"] and x["parent"] == child["parent"], explored)
            if len(list(existsInExplored)):
                continue

            child["level"] = count
            child["cost"] = count + manhattan(goal, child["current"])
            betterFrontier = filter(lambda x: x["level"] == child["level"] and x["cost"] < child["cost"], frontier)
            betterExplored = filter(lambda x: x["level"] == child["level"] and x["cost"] < child["cost"], explored)
            #if not len(list(betterFrontier)) and not len(list(betterExplored)):
            if len(list(betterFrontier)):
                continue
            #if len(list(betterExplored)):
            #    continue
                
            frontier.append(child)


        existsInExplored = filter(lambda x: x["current"] == current["current"] and x["parent"] == current["parent"], explored)
        if not len(list(existsInExplored)):
            explored.append(current)
        else:
            #return explored
...
def main():
    for initialStateIndex in range(0, len(puzzles)):
        ...
        if display(result) =="G" or display(result) =="X":
            ...
            print(str(initialStateIndex) + ": " + initialState + " len " + str(len(display(result)) + 1)
              + " in " + str(run_time) + "s:" + display(result))
