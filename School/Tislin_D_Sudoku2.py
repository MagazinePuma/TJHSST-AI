...
def sudoku_neighbors(csp_table): # {0:[0, 1, 2, 3, 4, ...., 8, 9, 18, 27, 10, 11, 19, 20], 1:
  ''' Your code goes here '''
  nbrs = {x: set(csp_table[x // 9] + csp_table[(x % 9) + 9] + find_grid(csp_table, x)) for x in range(81)}    # csp_table[(x % 3) + 18]
  for val in nbrs:
...
def main():
   ...
   for line, puzzle in enumerate(puzzles):
      ...
      if solution == None:
          ...
          #print ("{}{} {}".format(" "*(len(str(line))+2), "No solution found.", checksum(puzzle)))
          continue
