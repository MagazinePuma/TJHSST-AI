...
def a_star(start, goal):
    ...
    frontier.push((0, 0, start, None, "", ""))
    explored = {}

    frontier_back = HeapPriorityQueue()
    frontier_back.push((0, 0, goal, None, "", ""))
    #explored_back = {}
    explored_back = {}


    while not frontier.isEmpty() or not frontier_back.isEmpty():
        cost, level, current, parent, compact, short_compact = frontier.pop()
        ...
        explored[current] = (parent, short_compact)

        for child, child_compact in generate_children(current):
            if child == goal:
                return extract_path(parent, explored, goal)
                #return extract_copact_path(path_array)
            ...
            if child in explored:
                continue
                #if explored[child][0] <= child_cost and explored[child][1] <= child_level:
                #if len(explored[child][2]) > len(new_child_compact):
                #    continue
                #explored[child] = (child_cost, child_level, new_child_compact)
            frontier.push((child_cost, child_level, child, current, compact, child_compact))

            if child in explored_back:
                #print("+++++> front catch ")
                #print("+++++> front side: " + new_child_compact)
                #print("+++++> back side: " + explored_back[child][1])
                #print("+++++> back side reverted: " + revert_path(explored_back[child][1]))
                #return new_child_compact + revert_path(explored_back[child][2])
                left_path = extract_path(current, explored, goal) + child_compact
                right_path = extract_path(child, explored_back, start)[::-1]
                #print("+++++> front left_path: " + left_path)
                #print("+++++> back right_path reverted: " + revert_path(right_path))
                return left_path + revert_path(right_path)

       
       
        cost_back, level_back, current_back, parent_back, compact_back, short_compact_back = frontier_back.pop()
        ...
        explored_back[current_back] = (parent_back, short_compact_back)

        for child_back, child_compact_back in generate_children(current_back):

            if child_back == start:
                return extract_path(parent_back, explored_back, start)
                #return extract_copact_path(path_array)
            ...
            #if child_back not in explored_back:
                #continue
                #if explored_back[child_back][0] <= child_cost_back and explored_back[child_back][1] <= child_level_back:
                #    continue
                #explored_back[child_back] = (child_cost_back, child_level_back, new_child_compact_back)
            frontier_back.push((child_cost_back, child_level_back, child_back, current_back, compact_back, child_compact_back))

            #if child_back in explored:
            #    left_path = extract_path(child_back, explored, goal)
            #    right_path = extract_path(parent_back, explored_back, start)
            #    return left_path + revert_path(right_path)
